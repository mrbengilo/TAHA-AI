...
import { compressImageToJpeg, GENERATED_IMAGE_MAX_BYTES } from "../../../../../lib/image-compression";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await compressImageToJpeg(source, GENERATED_IMAGE_MAX_BYTES);
    const bytes = new Uint8Array(await result.blob.arrayBuffer());
    if (bytes[0] !== 0xff || bytes[1] !== 0xd8 || bytes.length >= GENERATED_IMAGE_MAX_BYTES) throw new Error("IMAGE_PROBE_FAILED");
