...
export async function productSources(productId: string, override?: ProductDatabase) {
  ...
  const rows = await db.prepare(`SELECT m.id, m.external_id, m.metadata_json, m.source_connection_id, m.mime_type, m.byte_size
    ...
    ORDER BY CASE pm.role WHEN 'primary' THEN 0 ELSE 1 END, pm.sort_order, pm.created_at`).bind(productId, TAHA_WORKSPACE_ID).all<SourceImage>();
...
type DerivedImage = SourceImage & { origin: string; storage_provider: string; product_id: string; role: string };
...
export async function verifiedProductOptimizedImages(productId: string, override?: ProductDatabase) {
  ...
  const rows = await db.prepare(`SELECT m.id, m.external_id, m.metadata_json, m.source_connection_id,
    ...
    .bind(TAHA_WORKSPACE_ID, productId, ORIGINAL_IMAGE_MAX_BYTES, IMAGE_COMPRESSION_POLICY).all<DerivedImage>();
  const verified: Array<DerivedImage & { source_media_id: string }> = [];
  const seen = new Set<string>();
