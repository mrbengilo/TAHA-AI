...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  if (!isOperatorRequest(request)) return fail("UNAUTHORIZED", "Bạn cần đăng nhập để hủy công việc tự động.", 401);
  try {
    ...
    return fail("AUTOMATION_CANCEL_FAILED", "Không thể hủy công việc tự động.", 500);
  }
}
