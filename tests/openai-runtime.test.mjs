...
test("OpenAI request options work in the deployed workerd runtime and reject redirects", async () => {
  const sources = await Promise.all(["shoe-content", "facebook-content", "openai"].map((name) => readFile(new URL(`../lib/ai/${name}.ts`, import.meta.url), "utf8")));
  const source = sources.join("\n")
    .replace(/import\s+\{[^}]+\}\s+from\s+"\.\/(?:shoe-content|facebook-content)";/g, "")
