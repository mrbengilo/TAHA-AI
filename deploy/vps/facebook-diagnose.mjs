...
async function main() {
  const [run] = query("SELECT r.id, r.status, p.base_sku AS sku FROM automation_runs r JOIN products p ON p.id=r.product_id AND p.workspace_id=r.workspace_id WHERE r.request_key='trial:drive-only-facebook-v2' AND r.workspace_id='00000000-0000-4000-8000-000000000001'");
  if (!run || !/^[a-f0-9-]+$/.test(run.id)) throw new Error("TRIAL_NOT_FOUND");
  const jobs = query(`SELECT j.id, j.status, j.error_code, j.external_post_id, j.external_url FROM publish_jobs j JOIN schedules s ON s.id=j.schedule_id AND s.workspace_id=j.workspace_id WHERE s.created_by='automation:${run.id}'`);
  report("FACEBOOK_TRIAL", { ...run, jobs });
  const [connection] = query(`SELECT c.external_account_id, c.display_name, c.status, c.publish_mode, c.config_json, c.auth_ciphertext, c.auth_iv FROM channel_connections c JOIN schedules s ON s.connection_id=c.id AND s.workspace_id=c.workspace_id WHERE c.provider='facebook' AND s.created_by='automation:${run.id}' LIMIT 1`);
  ...
  const posts = await graph("FACEBOOK_RECENT_POSTS_HTTP", `${pageId}/published_posts`, { fields: "id,permalink_url,message,created_time", limit: 10 });
  if (Array.isArray(posts?.data)) report("FACEBOOK_MATCHING_RECENT_POSTS", posts.data.filter((post) => String(post.message ?? "").includes(run.sku)).map((post) => ({ id: post.id, url: post.permalink_url, createdAt: post.created_time })));
}
