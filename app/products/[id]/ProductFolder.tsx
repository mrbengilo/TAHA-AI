...
export default function ProductFolder({ productId }: { productId: string }) {
  ...
  async function confirm() {
    ...
    try {
      await api("/api/automation-runs", { method: "POST", body: JSON.stringify({ productId, targetProviders: ["facebook"], idempotencyKey: requestKey.current }) });
      setNotice("Đã xác nhận. Hệ thống dùng toàn bộ ảnh gốc đúng SKU, viết bài không giá, chọn hashtag và lên lịch Facebook. Bạn có thể đóng trang.");
      await refresh();
  ...
  return <div className="sku-folder">
    <section className="ui-page-header"><div className="ui-page-header-copy"><span className="ui-eyebrow">THƯ MỤC SKU {folder.product.base_sku}</span><h1>{folder.product.name}</h1><p>Google Sheets · SKU {folder.product.base_sku} · Ảnh và bài viết của cùng sản phẩm</p></div>
      {folder.canReview && <button type="button" className="ui-button is-primary" aria-busy={busy === "confirm"} disabled={!!busy || !!active || !!scheduled || !!sending || !!folder.validationError} onClick={() => void confirm()}>{busy === "confirm" ? "Đang xác nhận…" : active ? "Đang tối ưu ảnh và viết bài…" : scheduled || sending ? "Đã có lịch đăng tự động" : "Xác nhận · Dùng toàn bộ ảnh gốc và viết bài"}</button>}
    </section>
